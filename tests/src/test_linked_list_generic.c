...
DECLARE_LINKED_LIST_C(long)

LinkedList$long *linked_list;

void setUp(void) { linked_list = linked_list$long_new(); }

void tearDown(void) { linked_list$long_free(linked_list); }
...
void test_linked_list_pop_front() {
  ...
  for (long i = 0; i < 64; i++) {
    TEST_ASSERT_EQUAL_INT(EXIT_SUCCESS,
                          linked_list$long_pop_front(linked_list, &buf));
...
void test_linked_list_pop_back() {
  ...
  for (long i = 63; i >= 0; i--) {
    TEST_ASSERT_EQUAL_INT(EXIT_SUCCESS,
                          linked_list$long_pop_back(linked_list, &buf));
    TEST_ASSERT_EQUAL_INT(i, buf);
  }
  TEST_ASSERT_EQUAL_INT(0, linked_list$long_len(linked_list));
}
...
void test_linked_list_remove() {
  ...
  TEST_ASSERT_EQUAL_INT(EXIT_FAILURE,
                        linked_list$long_remove(linked_list, 63, &buf));
...
void test_linked_list_remove_if() {
  ...
  TEST_ASSERT_EQUAL_INT(10, linked_list$long_len(linked_list));
  linked_list$long_remove_if(linked_list, greater_eq_10);
  TEST_ASSERT_EQUAL_INT(10, linked_list$long_len(linked_list));
}
