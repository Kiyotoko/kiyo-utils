...
void vec_push(Vec* linked_list, void* e) {
    ...
    memcpy(linked_list->data + linked_list->len * linked_list->element_size, e, linked_list->element_size);
    linked_list->len++;
}
...
void vec_grow(Vec* linked_list) {
    ...
    void* array = realloc(linked_list->data, new_capacity * linked_list->element_size);
...
void vec_shrink(Vec* linked_list, size_t new_capacity) {
    if (new_capacity < linked_list->len) return;
    void* array = realloc(linked_list->data, new_capacity * linked_list->element_size);
    // Failed to allocate memory, return.
    if (!array) return;
